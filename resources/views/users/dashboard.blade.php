...
@section('content')
    ...
    <div class="optimize-section">
        <div class="optimize-left">
            ...
            <p>Sync your task data and refresh available commissions. Run daily for best results and to unlock the latest
                task assignments for your membership level.</p>
            <a href="{{ route('generate.order') }}" class="btn-optimize" id="optimizeBtn"
        ...
        <div class="optimize-meter">
            ...
            <div class="gauge-wrap">
                <div class="gauge-canvas">
                    <canvas id="taskGauge" data-done="{{ $gaugeDone }}" data-limit="{{ $gaugeLimit }}"
                        data-avail="{{ $gaugeAvailable }}" data-comm="{{ $gaugeCommission }}"></canvas>
    ...
    <div class="earn-grid">
        <div class="earn-card green">
            ...
            <div class="earn-card-value green">
                @if (!empty($userData['overpriced_amount']) && $userData['overpriced_amount'] > 0)
                    <span style="font-size:14px;color:var(--danger);">⚠ Hold:
                        ${{ number_format($userData['overpriced_amount'], 2) }}</span>
                @else
        ...
        <div class="earn-card violet">
            ...
            <div class="earn-card-sub">Limit:
                {{ $userData['order_limit'] ?? ($userData['membership_level']->task_limit ?? 'N/A') }}</div>
        </div>
    </div>
            ...
            <div class="card-header">
                ...
                <div class="chart-controls">
                    ...
                    <div class="date-range-picker" id="dateRangePicker">
                        <button type="button" class="date-range-btn" id="dateRangeBtn">📅 <span id="dateRangeLabel">Last 7
                                days</span></button>
            ...
            <div class="card-body">
                <div class="quick-stats">
                    <div class="qs-row">
                        ...
                        <span class="qs-value"
                            style="color:var(--success);">{{ $userData['total_today_orders'] ?? 0 }}</span>
                    </div>
                    <div class="qs-row">
                        <span class="qs-label">Membership</span>
                        <span class="qs-value"
                            style="color:var(--warning);">{{ $userData['membership_level']->level_name ?? 'Member' }}</span>
                    </div>
                    <div class="qs-row">
                    ...
                    <div class="qs-row">
                        ...
                        <span class="qs-value"
                            style="color:var(--warning);">{{ $userData['tasks_remaining'] ?? 'N/A' }}</span>
                    </div>
                    <div class="qs-row">
                        <span class="qs-label">Total Commission</span>
                        <span class="qs-value"
                            style="color:var(--success);">${{ number_format($userData['total_commission'] ?? 0, 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse(($userData['recent_tasks'] ?? []) as $order)
                            ...
                            <tr>
                                ...
                                <td><span
                                        class="task-badge {{ $order->type == 'Selected' ? 'selected-badge' : 'normal-badge' }}">{{ $order->type ?? 'Normal' }}</span>
                                </td>
                                ...
                                <td style="color:var(--text-muted);font-size:12px;">
                                    {{ optional($order->created_at)->format('M j, Y') ?? '' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="table-empty">No task activity yet — hit <strong>Start
                                        Optimization</strong> to get your first task.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <script>
            ...
            (function() {
                    ...
                    earningsChartInstance = new Chart(canvas.getContext('2d'), {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                backgroundColor: type === 'pie' ? PALETTE : (type === 'line' ?
                                    'rgba(20,168,0,0.15)' : PALETTE[0]),
                                fill: type === 'line'
                            }]
                        },
                        options: {
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    lastLabels = weeklyEarnings.map(function(d) {
                        return d.label;
                    });
                    lastData = weeklyEarnings.map(function(d) {
                        return d.amount;
                    });
                    renderChart(lastLabels, lastData, currentChartType);
                    ...
                    if (chartTypeTabs) {
                        chartTypeTabs.querySelectorAll('.pill-tab').forEach(function(tab) {
                            tab.addEventListener('click', function() {
                                chartTypeTabs.querySelectorAll('.pill-tab').forEach(function(t) {
                    ...
                    if (dateRangeBtn && dateRangeDropdown) {
                        dateRangeBtn.addEventListener('click', function(e) {
                            e.stopPropagation();
                            dateRangeDropdown.style.display = dateRangeDropdown.style.display === 'block' ?
                                'none' : 'block';
                        });
                    ...
                    if (presetsList) {
                        presetsList.querySelectorAll('li').forEach(function(item) {
                            item.addEventListener('click', function() {
                                presetsList.querySelectorAll('li').forEach(function(li) {
